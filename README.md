<!-- HEADER WAVE -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=200&color=gradient&customColorList=6,11,20&section=header&text=Túlio%20Santos&fontSize=60&fontColor=fff&fontAlignY=38&desc=Software%20Engineering%20Student%20%7C%20Developer%20%7C%20AI%20Enthusiast&descAlignY=58&descSize=20&descColor=ffffff&animation=fadeIn" width="100%"/>
</div>

<!-- TYPING ANIMATION -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=4079A8&center=true&vCenter=true&random=false&width=600&lines=Ol%C3%A1%2C+mundo!+%F0%9F%91%8B+Me+chamo+T%C3%BAlio;Estudante+de+Engenharia+de+Software;Desenvolvedor+Full+Stack;Entusiasta+de+IA+%26+LangChain;Sempre+aprendendo%2C+sempre+evoluindo+%F0%9F%9A%80" alt="Typing SVG" />
</div>

<br/>

<!-- ABOUT ME -->
<div align="center">

## Sobre mim

</div>

```python
class Tulio:
    def __init__(self):
        self.name        = "Túlio Santos"
        self.role        = "Estudante de Engenharia de Software"
        self.school      = "IFPE — Instituto Federal de Pernambuco"
        self.location    = "Pernambuco, Brasil 🇧🇷"
        self.email       = "sssantostulio@gmail.com"
        self.languages   = ["Python", "JavaScript", "C#"]
        self.frameworks  = [".NET", "LangChain", "RAG"]
        self.interests   = ["IA Generativa", "Web Dev", "Automação"]
        self.currently   = "Aprofundando em LangChain e Inteligência Artificial"

    def say_hi(self):
        print("Valeu por visitar meu perfil! Bora codar juntos? ")

me = Tulio()
me.say_hi()
```

<br/>

<!-- STATS CARDS -->
<div align="center">

## 📊 Estatísticas do GitHub

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=devtul1o&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0d1117&title_color=4079a8&icon_color=4079a8&text_color=c9d1d9"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=devtul1o&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=4079a8&text_color=c9d1d9"/>

</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=devtul1o&theme=tokyonight&hide_border=true&background=0d1117&stroke=4079a8&ring=4079a8&fire=ff6b6b&currStreakLabel=4079a8&sideLabels=c9d1d9&dates=8b949e" width="70%"/>
</div>

<br/>

<!-- ACTIVITY GRAPH -->
<div align="center">

## Gráfico de Atividade

[![Ashutosh's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=devtul1o&bg_color=0d1117&color=4079a8&line=4079a8&point=ffffff&area=true&hide_border=true)](https://github.com/devtul1o)

</div>

<br/>

<!-- SKILLS -->
<div align="center">

## Tecnologias & Ferramentas

### 💻 Linguagens
![JavaScript](https://img.shields.io/badge/JavaScript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![C#](https://img.shields.io/badge/C%23-%23239120.svg?style=for-the-badge&logo=csharp&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)

### ⚙️ Frameworks & Bibliotecas
![.NET](https://img.shields.io/badge/.NET-5C2D91?style=for-the-badge&logo=.net&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white)

### 🤖 IA & Machine Learning
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Hugging Face](https://img.shields.io/badge/Hugging%20Face-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)

### 🧰 Ferramentas & IDEs
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![PyCharm](https://img.shields.io/badge/PyCharm-143?style=for-the-badge&logo=pycharm&logoColor=black&color=black&labelColor=green)
![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)

</div>

<br/>

<!-- SKILL LEVEL BARS -->
<div align="center">

## Nível de Conhecimento

</div>

<div  align="center">

| Tecnologia | Nível | Barra de Progresso |
|:----------:|:-----:|:------------------:|
| HTML / CSS | Intermediário | `████████░░` 80% |
| JavaScript | Intermediário | `███████░░░` 70% |
| Python | Intermediário | `███████░░░` 70% |
| C# / .NET | Básico/Intermediário | `█████░░░░░` 50% |
| LangChain | Básico/Intermediário | `█████░░░░░` 50% |
| RAG (IA) | Em desenvolvimento | `████░░░░░░` 40% |

</div>

<br/>

<!-- TROPHIES -->
<div align="center">

## Troféus GitHub

<img src="https://github-profile-trophy.vercel.app/?username=devtul1o&theme=tokyonight&no-frame=true&no-bg=true&margin-w=4&row=1" width="100%" />

</div>


<!-- CONTACT -->
<div align="center">

## Formas de Contato

[![Instagram](https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/tulio_)
[![Gmail](https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sssantostulio@gmail.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/túlio-santos-78a808217/)

</div>

<br/>

<!-- PROFILE VIEWS -->
<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=devtul1o&color=4079a8&style=for-the-badge&label=VISITAS+AO+PERFIL)

</div>

<!-- FOOTER WAVE -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=120&color=gradient&customColorList=6,11,20&section=footer" width="100%"/>
</div>
